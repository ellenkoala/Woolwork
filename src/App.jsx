...
import { useSpinning } from "./hooks/useSpinning.js";
import { SpinningView } from "./components/SpinningView.jsx";
...
export default function KnittingApp() {
  ...
  // ── Spinning ──────────────────────────────────────────────────────────
  // Owns its own state and handlers; spinProjects/activeSpinId/activeSpinProject
  // are pulled out because persistence and the shared export flow read them.
  const spinning = useSpinning(modalData, closeModal);
  const {
    spinProjects, setSpinProjects, activeSpinId, setActiveSpinId,
    spinView, setSpinView, editingSpinProject, setEditingSpinProject,
    spinSearch, setSpinSearch, spinFilterStatus, setSpinFilterStatus,
    spinLogDate, setSpinLogDate, spinLogHours, setSpinLogHours,
    spinLogGSpun, setSpinLogGSpun, spinLogNote, setSpinLogNote,
    spinPhotoInputRef, activeSpinProject, filteredSpinProjects,
  ...
  },[session, setNeedleLibrary, setEquipLibrary, setYarnLibrary, setFibreLibrary, setSpinProjects]);
  ...
  return (
    <div style={{minHeight:"100vh",background:C.bg,fontFamily:"'Georgia','Times New Roman',serif",color:C.text,userSelect:"none"}} onMouseUp={handleMouseUp}>
      ...
      <div style={{padding:"16px 24px"}}>
        ...
        {/* ═══ SPINNING ════════════════════════════════════════════════ */}
        {appMode==="spinning"&&(
          <SpinningView theme={C} btnPrimary={btnPrimary} btnSecondary={btnSecondary} btnDanger={btnDanger}
            inp={inp} lbl={lbl} modal={modal} modalData={modalData} setModalData={setModalData}
            openModal={openModal} closeModal={closeModal}
            allSpinStatuses={allSpinStatuses} allFiberTypes={allFiberTypes} allSpinTools={allSpinTools}
            spinView={spinView} setSpinView={setSpinView}
            editingSpinProject={editingSpinProject} setEditingSpinProject={setEditingSpinProject}
            spinSearch={spinSearch} setSpinSearch={setSpinSearch}
            spinFilterStatus={spinFilterStatus} setSpinFilterStatus={setSpinFilterStatus}
            spinLogDate={spinLogDate} setSpinLogDate={setSpinLogDate}
            spinLogHours={spinLogHours} setSpinLogHours={setSpinLogHours}
            spinLogGSpun={spinLogGSpun} setSpinLogGSpun={setSpinLogGSpun}
            spinLogNote={spinLogNote} setSpinLogNote={setSpinLogNote}
            spinPhotoInputRef={spinPhotoInputRef}
            spinProjects={spinProjects} setSpinProjects={setSpinProjects}
            activeSpinId={activeSpinId} setActiveSpinId={setActiveSpinId}
            activeSpinProject={activeSpinProject} filteredSpinProjects={filteredSpinProjects}
            updateSpinProject={updateSpinProject} saveSpinProject={saveSpinProject}
            addSpinLogEntry={addSpinLogEntry} addSpinPhoto={addSpinPhoto}
          />
        )}
